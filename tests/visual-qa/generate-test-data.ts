...
async function main() {
  ...
  const cashAccount = await prisma.cashBankAccount.upsert({
    where: { workspaceId_accountId: { workspaceId: workspace.id, accountId: (await getAccountByCode(workspace.id, "CASH_IN_HAND")).id } },
    create: { workspaceId: workspace.id, accountId: (await getAccountByCode(workspace.id, "CASH_IN_HAND")).id, name: "Main Cash", openingBalance: 50000, currentBalance: 50000, isBank: false, isActive: true },
    update: {},
  });

  const bankAccount = await prisma.cashBankAccount.upsert({
    where: { workspaceId_accountId: { workspaceId: workspace.id, accountId: (await getAccountByCode(workspace.id, "BANK_ACCOUNT")).id } },
    create: { workspaceId: workspace.id, accountId: (await getAccountByCode(workspace.id, "BANK_ACCOUNT")).id, name: "Main Bank - HBL", openingBalance: 1000000, currentBalance: 1000000, isBank: true, bankName: "Habib Bank Limited", accountNumber: "0012-7901-1234-56", accountTitle: "Visual QA Company Ltd.", isActive: true },
    update: {},
  });

  // Create customers
  const customer1 = await prisma.customer.upsert({
    where: { id: "visual-qa-customer-1" },
    create: { id: "visual-qa-customer-1", workspaceId: workspace.id, name: "Acme Corporation Pvt Ltd", companyName: "Acme Corporation Pvt Ltd", address: "456 Business Park, Shahrah-e-Faisal", city: "Karachi", phone: "+92 21 3456 7890", email: "billing@acme.com", currentBalance: 0, creditLimit: 5000000, status: "ACTIVE" },
    update: {},
  });

  const customerLongName = await prisma.customer.upsert({
    where: { id: "visual-qa-customer-long" },
    create: { id: "visual-qa-customer-long", workspaceId: workspace.id, name: "Extremely Long Company Name That Tests Text Wrapping And Layout Stability In The Invoice Header And Address Sections Private Limited", companyName: "Extremely Long Company Name That Tests Text Wrapping And Layout Stability In The Invoice Header And Address Sections Private Limited", address: "Very Long Address Line That Spans Multiple Lines To Test Wrapping Behavior In The Invoice Layout And Address Block Rendering System Karachi Pakistan 75500", city: "Karachi", phone: "+92 21 3456 7890", email: "accounts@verylongcompany.com", currentBalance: 0, creditLimit: 10000000, status: "ACTIVE" },
    update: {},
  });
  ...
  const productWeight = await prisma.product.upsert({
    ...
    create: { id: "visual-qa-product-weight", workspaceId: workspace.id, name: "Basmati Rice Premium Grade", sku: "RICE-BAS-PREM", description: "Premium grade aged basmati rice", category: "Food Grains", unit: "KG", unitWeight: 1, stockQuantity: 5000, costPrice: 180, sellingPrice: 280, reorderLevel: 500, status: "ACTIVE" },
    update: {},
  });

  const productWeightHeavy = await prisma.product.upsert({
    where: { id: "visual-qa-product-heavy" },
    create: { id: "visual-qa-product-heavy", workspaceId: workspace.id, name: "Steel Reinforcement Bars 20mm", sku: "STEEL-REBAR-20", description: "Grade 60 steel reinforcement bars for construction", category: "Construction Materials", unit: "KG", unitWeight: 2.47, stockQuantity: 10000, costPrice: 220, sellingPrice: 285, reorderLevel: 1000, status: "ACTIVE" },
    update: {},
  });

  // Create suppliers
  const supplier1 = await prisma.supplier.upsert({
    where: { id: "visual-qa-supplier-1" },
    create: { id: "visual-qa-supplier-1", workspaceId: workspace.id, name: "Global Steel Industries Ltd", companyName: "Global Steel Industries Ltd", address: "789 Industrial Estate, Port Qasim", city: "Karachi", phone: "+92 21 9876 5432", email: "orders@globalsteel.com", currentBalance: 0, status: "ACTIVE" },
    update: {},
  });

  const supplierLong = await prisma.supplier.upsert({
    where: { id: "visual-qa-supplier-long" },
    create: { id: "visual-qa-supplier-long", workspaceId: workspace.id, name: "Extremely Long Supplier Name That Tests Procurement Document Layout And Supplier Address Block Rendering In Purchase Orders And Goods Receipt Notes Private Limited", companyName: "Extremely Long Supplier Name That Tests Procurement Document Layout And Supplier Address Block Rendering In Purchase Orders And Goods Receipt Notes Private Limited", address: "Very Long Supplier Address That Spans Multiple Lines To Test Wrapping In PO And GRN Documents Industrial Area Sector 7-B Karachi Pakistan 75010", city: "Karachi", phone: "+92 21 9876 5432", email: "procurement@verylongsupplier.com", currentBalance: 0, status: "ACTIVE" },
    update: {},
  });
  ...
  // Output IDs for visual QA script
  const ids = {
...
async function getAccountByCode(workspaceId: string, code: string) {
...
async function ensureDefaultAccounts(workspaceId: string) {
  const codes = [
    "CASH_IN_HAND", "BANK_ACCOUNT", "ACCOUNTS_RECEIVABLE", "ACCOUNTS_PAYABLE",
    "INVENTORY", "COST_OF_GOODS_SOLD", "SALES_REVENUE", "OFFICE_EXPENSE",
    "OTHER_INCOME", "SALES_TAX_PAYABLE", "WITHHOLDING_TAX_PAYABLE",
...
function getNormalBalance(code: string) {
