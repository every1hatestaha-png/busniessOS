...
describe("supplier return cancellation", () => {
  beforeAll(async () => {
    ...
    const purchase = await db.purchaseOrder.create({ data: { workspaceId, supplierId, orderNumber: `PO-RET-${runId}`, status: "COMPLETED", totalAmount: 100, paidAmount: 0, balanceAmount: 80 } });
