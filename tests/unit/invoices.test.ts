...
describe("getInvoice", () => {
  it("queries and maps the per-unit discount without changing stored invoice totals", async () => {
    ...
    mocks.queryRaw.mockImplementation(async (strings: TemplateStringsArray, ...values: unknown[]) => {
      ...
      if (sql.includes('FROM "sales_order_items" soi')) {
        ...
        return [{
          ...
          productSku: "SAVED-SKU", fallbackSku: "CURRENT-SKU", quantity: new Prisma.Decimal(50),
          unitPrice: new Prisma.Decimal(1000), discountPerUnit: new Prisma.Decimal(50),
          totalPrice: new Prisma.Decimal(47500),
        }];
      }
    ...
    expect(invoice?.order).toEqual({
      ...
      items: [{ id: "item_1", name: "Saved product", sku: "SAVED-SKU", quantity: 50, unitPrice: 1000, discountPerUnit: 50, total: 47500 }],
    });
