...
export async function listInvoices(workspaceId: string) {
  ...
  return rows.map((row) => ({ id: row.id, invoiceNumber: row.invoiceNumber, customerName: row.customer.companyName ?? row.customer.name, date: row.issuedAt.toISOString(), dueDate: row.dueDate?.toISOString() ?? null, total: Number(row.amount), paid: Number(row.paidAmount), balance: row.status === "CANCELLED" ? 0 : Number(row.amount.minus(row.paidAmount).minus(row.creditApplied)), status: row.dueDate && row.dueDate < new Date() && ["UNPAID", "PARTIALLY_PAID"].includes(row.status) ? "OVERDUE" as const : row.status, orderNumber: row.salesOrder?.orderNumber ?? "-" }));
}
...
export async function getInvoice(workspaceId: string, id: string) {
  ...
  const [items, directPayments, allocations] = await Promise.all([
    invoice.salesOrderId ? db.$queryRaw<Array<{ id: string; productName: string | null; fallbackProductName: string; productSku: string | null; fallbackSku: string | null; quantity: number; unitPrice: Prisma.Decimal; discountPerUnit: Prisma.Decimal; totalPrice: Prisma.Decimal }>>`
      SELECT soi."id", soi."productName", p."name" AS "fallbackProductName", soi."productSku", p."sku" AS "fallbackSku", soi."quantity", soi."unitPrice", soi."discountPerUnit", soi."totalPrice"
