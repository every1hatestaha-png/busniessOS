...
export async function getInvoice(workspaceId: string, id: string) {
  ...
  const [items, directPayments, allocations] = await Promise.all([
    invoice.salesOrderId ? db.$queryRaw<Array<{ id: string; productName: string | null; fallbackProductName: string; productSku: string | null; fallbackSku: string | null; unit: string; quantity: number; unitPrice: Prisma.Decimal; discountPerUnit: Prisma.Decimal; totalPrice: Prisma.Decimal }>>`
      SELECT soi."id", soi."productName", p."name" AS "fallbackProductName", soi."productSku", p."sku" AS "fallbackSku", p."unit"::text AS "unit", soi."quantity", soi."unitPrice", soi."discountPerUnit", soi."totalPrice"
