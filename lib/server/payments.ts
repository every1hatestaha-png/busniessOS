...
import { postCustomerReceiptWithWht } from "@/lib/server/customer-payment-accounting";
  ...
  return withSerializableRetry(async (tx) => {
    if (data.idempotencyKey) {
      ...
      if (existing) {
        ...
        if (existing.supplierId || existing.customerId !== data.customerId || !existing.amount.equals(data.amount) || !existing.withholdingTaxAmount.equals(data.withholdingTaxAmount ?? 0) || existing.cashBankAccountId !== data.cashBankAccountId || existing.method !== data.method || !sameAllocations) throw new PaymentDomainError("This idempotency key was already used for a different payment request.");
    ...
    if (withholdingTaxAmount.greaterThan(amount)) throw new PaymentDomainError("Withholding tax cannot exceed the gross settlement amount.");
    if (netAmount.lessThan(0)) throw new PaymentDomainError("Net receipt cannot be negative.");
    ...
    if (requestedAllocations.length) {
      ...
      if (!allocationTotal.equals(amount)) throw new PaymentDomainError("Payment allocations must equal the gross settlement amount.");
    ...
    await tx.ledgerEntry.create({ data: { workspaceId: context.workspaceId, customerId: customer.id, type: "PAYMENT_RECEIVED", credit: amount, description: `Payment ${paymentNumber}${withholdingTaxAmount.greaterThan(0) ? `, WHT ${withholdingTaxAmount.toFixed(2)}` : ""}`, referenceId: payment.id, date: data.paymentDate } });
    await tx.customer.update({ where: { id: customer.id, workspaceId: context.workspaceId }, data: { currentBalance: { decrement: amount } } });
    await postCustomerReceiptWithWht(tx, { workspaceId: context.workspaceId, paymentId: payment.id, documentNo: paymentNumber, date: data.paymentDate, grossAmount: amount, withholdingTaxAmount, cashBankAccountId: cashBankAccount.id });
    for (const allocation of requestedAllocations) {
      const invoice = invoices.find((entry) => entry.id === allocation.invoiceId)!;
    ...
    await writeAudit(tx, { workspaceId: context.workspaceId, actorId: context.userId, action: "customer.payment_recorded", entityType: "Payment", entityId: payment.id, metadata: { grossAmount: data.amount, withholdingTaxAmount: data.withholdingTaxAmount ?? 0, netAmount: netAmount.toNumber() } });
...
export async function reverseCustomerPayment(context: ServiceContext, paymentId: string, reason: string) {
  ...
  return withSerializableRetry(async (tx) => {
    ...
    const netAmount = payment.netAmount ?? payment.amount.minus(payment.withholdingTaxAmount);
    ...
    await writeAudit(tx, {
      ...
      entityType: "Payment",
      entityId: payment.id,
      metadata: { reversalId: reversal.id, reversalNumber, reason: cleanReason, grossAmount: payment.amount.toString(), withholdingTaxAmount: payment.withholdingTaxAmount.toString(), netAmount: netAmount.toString() },
    });
