...
export const supplierPaymentSchema = z.object({
  ...
  // Optional at the input type boundary for backward compatibility; validation
  // below still requires at least one explicit settlement target at runtime.
  ...
  if (payment.allocations.length === 0) {
    ctx.addIssue({ code: "custom", path: ["allocations"], message: "Allocate this payment to an opening balance or one or more GRNs." });
  }
});
