    ...
    <div className="mx-auto max-w-[1400px] space-y-6 print:max-w-none print:space-y-0">
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px] print:block">
        <article data-document className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-none print:rounded-none print:border-0">
          ...
          <section data-document-totals className="flex justify-end border-t border-neutral-200 p-6 print:p-3 sm:p-8"><div className="w-full max-w-sm space-y-3 print:space-y-1 text-sm"><p className="hidden text-[9px] uppercase tracking-wide text-neutral-500 print:block">Invoice {invoice.invoiceNumber}</p>{invoice.order && <><div className="flex justify-between"><span className="text-neutral-500">Subtotal</span><span>{formatPKR(invoice.order.subtotal)}</span></div><div className="flex justify-between"><span className="text-neutral-500">Discount</span><span>- {formatPKR(invoice.order.discount)}</span></div></>}<div className="flex justify-between border-t pt-3 print:pt-1 text-base font-bold"><span>Invoice total</span><span>{formatPKR(invoice.total)}</span></div><div className="flex justify-between"><span className="text-neutral-500">Payments received</span><span className="text-emerald-700">{formatPKR(invoice.paid)}</span></div>{invoice.creditApplied > 0 && <div className="flex justify-between"><span className="text-neutral-500">Customer credit applied</span><span>{formatPKR(invoice.creditApplied)}</span></div>}<div className="flex justify-between rounded-lg bg-neutral-950 p-4 print:p-2 text-base font-bold text-white print:border print:border-neutral-300 print:bg-white print:text-black"><span>Balance due</span><span>{formatPKR(invoice.balance)}</span></div></div></section>
