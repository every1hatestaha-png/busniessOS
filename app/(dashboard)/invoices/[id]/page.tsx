      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_360px] print:block">
        <article data-document className="overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-none print:rounded-none print:border-0">
          ...
          <section className="overflow-x-auto">
            {invoice.order?.items.length ? <Table className="min-w-[650px] table-fixed print:min-w-0"><TableHeader><TableRow><TableHead className="w-[34%] pl-6 sm:pl-8">Description</TableHead><TableHead className="w-[16%]">SKU</TableHead><TableHead className="w-[8%] text-right">Qty</TableHead><TableHead className="w-[13%] text-right">Rate</TableHead><TableHead className="w-[14%] text-right">Disc/unit</TableHead><TableHead className="w-[15%] pr-6 text-right sm:pr-8">Amount</TableHead></TableRow></TableHeader><TableBody>{invoice.order.items.map((item) => { const { gross, discountTotal, total } = calculateSaleLine(item); return <TableRow key={item.id}><TableCell className="whitespace-normal break-words pl-6 font-medium sm:pl-8">{item.name}</TableCell><TableCell className="whitespace-normal break-all text-neutral-500">{item.sku || "-"}</TableCell><TableCell className="text-right">{item.quantity}</TableCell><TableCell className="text-right tabular-nums">{formatPKR(item.unitPrice)}</TableCell><TableCell className="text-right tabular-nums">{formatPKR(item.discountPerUnit)}</TableCell><TableCell className="pr-6 text-right font-semibold tabular-nums sm:pr-8">{formatPKR(total)}</TableCell></TableRow>; })}</TableBody></Table> : <p className="p-8 text-sm text-neutral-500">No sales order line items are linked to this invoice.</p>}
          </section>
          ...
          <section data-document-totals className="flex justify-end border-t border-neutral-200 p-6 print:p-3 sm:p-8"><div className="w-full max-w-sm space-y-3 print:space-y-1 text-sm"><p className="hidden text-[9px] uppercase tracking-wide text-neutral-500 print:block">Invoice {invoice.invoiceNumber}</p>{invoice.order && <><div className="flex justify-between"><span className="text-neutral-500">Subtotal</span><span>{formatPKR(invoice.order.subtotal)}</span></div><div className="flex justify-between"><span className="text-neutral-500">Discount</span><span>- {formatPKR(invoice.order.discount)}</span><span className="ml-2 text-[10px] text-neutral-400">(Per-unit)</span></div></>}<div className="flex justify-between border-t pt-3 print:pt-1 text-base font-bold"><span>Invoice total</span><span>{formatPKR(invoice.total)}</span></div><div className="flex justify-between"><span className="text-neutral-500">Payments received</span><span className="text-emerald-700">{formatPKR(invoice.paid)}</span></div>{invoice.creditApplied > 0 && <div className="flex justify-between"><span className="text-neutral-500">Customer credit applied</span><span>{formatPKR(invoice.creditApplied)}</span></div>}<div className="flex justify-between rounded-lg bg-neutral-950 p-4 print:p-2 text-base font-bold text-white print:border print:border-neutral-300 print:bg-white print:text-black"><span>Balance due</span><span>{formatPKR(invoice.balance)}</span></div></div></section>
