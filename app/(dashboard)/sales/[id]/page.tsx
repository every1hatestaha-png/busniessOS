...
import { findCustomer, findSale } from "@/lib/demo-data";
    ...
    <div className="mx-auto max-w-[1500px] space-y-6">
      ...
      <div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="space-y-6">
          <Card className="shadow-none"><CardHeader><CardTitle>Order items</CardTitle></CardHeader><CardContent className="px-0"><Table><TableHeader><TableRow><TableHead className="pl-4">Product</TableHead><TableHead className="text-right">Quantity</TableHead><TableHead className="text-right">Unit price</TableHead><TableHead className="text-right">Discount</TableHead><TableHead className="pr-4 text-right">Line total</TableHead></TableRow></TableHeader><TableBody>{sale.items.map((item) => <TableRow key={item.productId}><TableCell className="pl-4 font-medium">{item.productName}</TableCell><TableCell className="text-right">{item.quantity}</TableCell><TableCell className="text-right">{formatPKR(item.unitPrice)}</TableCell><TableCell className="text-right text-neutral-600">{formatPKR(item.discount)}</TableCell><TableCell className="pr-4 text-right font-semibold">{formatPKR(item.total)}</TableCell></TableRow>)}</TableBody></Table></CardContent></Card>

          <Card className="shadow-none"><CardHeader><CardTitle>Customer</CardTitle></CardHeader><CardContent>{customer ? <div className="grid gap-5 sm:grid-cols-2"><div><p className="font-semibold text-neutral-950">{customer.companyName}</p><p className="mt-1 text-sm text-neutral-500">{customer.name}</p></div><div className="space-y-2 text-sm text-neutral-600"><p className="flex items-center gap-2"><Phone className="h-4 w-4 text-neutral-400" />{customer.phone}</p><p className="flex items-center gap-2"><MapPin className="h-4 w-4 text-neutral-400" />{customer.address}</p></div><div className="grid grid-cols-2 gap-4 border-t pt-4 sm:col-span-2"><div><p className="text-xs text-neutral-500">Account balance</p><p className="mt-1 font-semibold">{formatPKR(customer.currentBalance)}</p></div><div><p className="text-xs text-neutral-500">Credit limit</p><p className="mt-1 font-semibold">{formatPKR(customer.creditLimit)}</p></div></div></div> : <p className="text-sm text-neutral-500">Customer details unavailable.</p>}</CardContent></Card>
