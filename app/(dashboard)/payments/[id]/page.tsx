...
export default async function PaymentReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const whatsappUrl = !isReversal ? buildWhatsAppShareUrl(
    receipt.customer.phone,
    `Assalam-o-Alaikum. ${workspace.name} received ${formatPKR(receipt.netAmount)} against gross settlement ${formatPKR(receipt.amount)} on receipt ${receipt.documentNumber}${receipt.withholdingTaxAmount > 0 ? ` (WHT ${formatPKR(receipt.withholdingTaxAmount)})` : ""}.`,
  ) : null;
    ...
    <div className="mx-auto max-w-[1400px] space-y-4 print:max-w-none print:space-y-0">
      <div className="flex flex-wrap items-center justify-between gap-3 print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {whatsappUrl && <a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex h-8 items-center rounded-lg border border-neutral-200 bg-white px-3 text-sm font-medium hover:bg-neutral-50">WhatsApp</a>}
          <PrintButton label={isReversal ? "Print reversal" : "Print receipt"} />
          {!isReversal && <PrintButton label="80mm receipt" format="thermal" />}
        </div>
      </div>
      <DocumentFrame workspace={workspace} title={documentTitle} number={receipt.documentNumber} status={status} statusReason={receipt.reversalOf?.documentNumber ? `Reversal of ${receipt.reversalOf.documentNumber}` : undefined} details={<><p>Date: {formatDate(receipt.paymentDate)}</p><p>Method: {receipt.method.replaceAll("_", " ")}</p></>}>
        ...
        <section data-document-totals className="my-6 flex justify-end"><div className="w-full max-w-sm space-y-2 text-sm"><div className="flex justify-between border-t border-neutral-300 pt-3 text-lg font-bold"><span>{isReversal ? "Amount reversed" : "Gross balance settled"}</span><span>{formatPKR(receipt.amount)}</span></div>{!isReversal && receipt.withholdingTaxAmount > 0 && <><div className="flex justify-between"><span className="text-neutral-600">Withholding tax deducted</span><span>{formatPKR(receipt.withholdingTaxAmount)}</span></div><div className="flex justify-between font-semibold"><span>Net cash/bank received</span><span>{formatPKR(receipt.netAmount)}</span></div></>}{!isReversal && receipt.allocations.length > 0 && <div className="flex justify-between"><span className="text-neutral-600">Allocated</span><span>{formatPKR(receipt.allocatedAmount)}</span></div>}{!isReversal && receipt.unallocatedAmount > 0 && <div className="flex justify-between"><span className="text-neutral-600">Credit on account</span><span>{formatPKR(receipt.unallocatedAmount)}</span></div>}</div></section>
