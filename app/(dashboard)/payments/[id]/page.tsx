...
export default async function PaymentReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const hasWithholdingTax = receipt.withholdingTaxAmount > 0;
  ...
  return (
    <div className="mx-auto max-w-[1400px] space-y-4 print:max-w-none print:space-y-0">
      ...
      <DocumentFrame workspace={workspace} title={documentTitle} number={receipt.documentNumber} status={status} statusReason={receipt.reversalOf?.documentNumber ? `Reversal of ${receipt.reversalOf.documentNumber}` : undefined} details={<><p>Date: {formatDate(receipt.paymentDate)}</p><p>Method: {receipt.method.replaceAll("_", " ")}</p></>}>
        ...
        <section data-document-totals className="my-6 flex justify-end"><div className="w-full max-w-sm space-y-2 text-sm"><div className="flex justify-between border-t border-neutral-300 pt-3 text-lg font-bold"><span>{isReversal ? "Gross amount reversed" : "Gross balance settled"}</span><span>{formatPKR(receipt.amount)}</span></div>{hasWithholdingTax && <div className="flex justify-between"><span className="text-neutral-600">Withholding tax</span><span>{formatPKR(receipt.withholdingTaxAmount)}</span></div>}{hasWithholdingTax && <div className="flex justify-between font-semibold"><span>{isReversal ? "Net cash/bank reversed" : "Net cash/bank received"}</span><span>{formatPKR(receipt.netAmount)}</span></div>}{!isReversal && receipt.allocations.length > 0 && <div className="flex justify-between"><span className="text-neutral-600">Allocated</span><span>{formatPKR(receipt.allocatedAmount)}</span></div>}{!isReversal && receipt.unallocatedAmount > 0 && <div className="flex justify-between"><span className="text-neutral-600">Credit on account</span><span>{formatPKR(receipt.unallocatedAmount)}</span></div>}</div></section>
