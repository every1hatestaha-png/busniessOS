...
import { DEMO_STOCK_MOVEMENTS, findProduct } from "@/lib/demo-data";
...
export default async function ProductDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!product) notFound();
  const movements = DEMO_STOCK_MOVEMENTS.filter((movement) => movement.productId === product.id);
    ...
    <main className="space-y-6 p-4 md:p-6 lg:p-8">
      <div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          ...
          <StockAdjustment initialStock={product.stockQuantity} unit={product.unit.toLowerCase()} />
        </div>
      </div>
          ...
          <CardContent className="p-0">
            <Table>
              ...
              <TableBody>
                ...
                {movements.length === 0 && <TableRow><TableCell colSpan={5} className="h-32 text-center text-neutral-500">No stock movements recorded for this demo product.</TableCell></TableRow>}
              </TableBody>
            </Table>
          </CardContent>
        </Card>
