  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6">
      ...
      <Card className="gap-0 py-0 shadow-none">
        ...
        <CardContent className="p-0"><div className="overflow-x-auto"><Table className="min-w-[760px]"><TableHeader className="bg-neutral-50/80"><TableRow><TableHead className="pl-4">Supplier</TableHead><TableHead>Contact</TableHead><TableHead className="text-right">Gross PO value</TableHead><TableHead className="text-right">Payable</TableHead><TableHead className="w-12"><span className="sr-only">Open</span></TableHead></TableRow></TableHeader><TableBody>{suppliers.map((supplier) => <TableRow key={supplier.id}><TableCell className="pl-4"><Link prefetch={false} className="font-medium underline-offset-4 hover:underline" href={`/suppliers/${supplier.id}`}>{supplier.companyName ?? supplier.name}</Link><p className="text-xs text-neutral-500">{supplier.name}</p></TableCell><TableCell><p>{supplier.phone ?? "-"}</p><p className="text-xs text-neutral-500">{supplier.email ?? ""}</p></TableCell><TableCell className="text-right tabular-nums text-neutral-600">{formatPKR(supplier.totalPurchases)}</TableCell><TableCell className="text-right font-semibold tabular-nums">{formatPKR(supplier.currentBalance)}</TableCell><TableCell className="pr-3 text-right"><Link prefetch={false} href={`/suppliers/${supplier.id}`} aria-label={`View ${supplier.name}`} className={cn(buttonVariants({ variant: "ghost", size: "icon-sm" }), "inline-flex")}><ArrowUpRight className="h-4 w-4" /></Link></TableCell></TableRow>)}{!suppliers.length && <TableRow><TableCell className="h-28 text-center text-neutral-500" colSpan={5}>No suppliers yet.</TableCell></TableRow>}</TableBody></Table></div></CardContent>
      </Card>
    </div>
  );
}
