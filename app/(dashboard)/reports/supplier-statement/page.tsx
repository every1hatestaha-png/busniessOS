...
export default async function SupplierStatementPage({ searchParams }: { searchParams: Query }) {
  ...
  return <ReportFrame workspace={workspace} title="Supplier Statement" from={statement?.from ?? from} to={statement?.to ?? to} subtitle={statement ? `${statement.party.displayName}${statement.party.phone ? ` | ${statement.party.phone}` : ""}` : "Select a supplier to generate a statement"} filters={filters}>{statement ? <StatementTable statement={statement} balanceLabel="Amount payable" /> : <div className="py-14 text-center"><p className="font-semibold">No supplier selected</p><p className="mt-1 text-sm text-neutral-500">Choose a supplier above to view persisted statement activity.</p></div>}</ReportFrame>;
}
