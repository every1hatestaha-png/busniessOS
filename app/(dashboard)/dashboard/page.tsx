...
export default async function DashboardPage() {
  ...
  const todaySales = summary.sales.filter((sale) => sale.date.startsWith(today));
  const monthSales = summary.sales.filter((sale) => sale.date.startsWith(month));
  const monthRevenue = monthSales.reduce((sum, sale) => sum + sale.total, 0);
  const salesReceivables = monthSales.reduce((sum, sale) => sum + sale.balance, 0);
  const collected = monthSales.reduce((sum, sale) => sum + sale.paid, 0);
  const collectionRate = monthRevenue > 0 ? Math.round((collected / monthRevenue) * 100) : 0;
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      <section className="overflow-hidden border bg-neutral-950 px-5 py-6 text-white md:px-7">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-neutral-400">Here is the latest pulse of {workspace.name}.</p>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard label="This month revenue" value={formatPKR(monthRevenue)} detail={`${monthSales.length} active sales orders`} icon={CircleDollarSign} />
        ...
        <Card className="shadow-none">
          <CardHeader className="flex-row items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-neutral-500">Newest persisted customer orders</p>
            </div>
              ...
              <TableBody>
                {summary.sales.slice(0, 5).map((sale) => (
                  <TableRow key={sale.id}>
                    ...
                    <TableCell className="text-right text-neutral-600">{formatPKR(sale.balance)}</TableCell>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          <Card className="shadow-none">
            ...
            <CardContent>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3 border-t pt-4 text-sm"><div><p className="text-neutral-500">Collected</p><p className="mt-1 font-semibold">{formatPKR(collected)}</p></div><div><p className="text-neutral-500">Outstanding</p><p className="mt-1 font-semibold">{formatPKR(salesReceivables)}</p></div></div>
          </CardContent>
        </Card>
