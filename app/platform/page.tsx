...
import { requirePlatformMfa } from "@/lib/server/platform-security";
          ...
          <header className="border-b border-[#e2e8e5] bg-white/90 px-5 py-5 backdrop-blur sm:px-7 lg:px-10">
            <div className="mx-auto flex max-w-[1500px] flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              ...
              <div className="inline-flex w-fit items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">
                <span className="size-2 rounded-full bg-emerald-500" /> MFA-protected owner session
              </div>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-[1500px] space-y-6 px-5 py-7 sm:px-7 lg:px-10">
            ...
            <section className="overflow-hidden rounded-xl border border-[#e2e8e5] bg-white shadow-sm">
              <div className="flex flex-col gap-1 border-b border-[#e2e8e5] px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
                ...
                <p className="text-xs text-slate-400">Sensitive actions require recent MFA reverification.</p>
              </div>
                ...
                <table className="min-w-full divide-y divide-slate-200 text-sm">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {workspaces.map((workspace) => {
                      const accessEnd = workspace.status === "TRIALING" ? workspace.trialEndsAt : workspace.currentPeriodEnd ?? workspace.graceEndsAt;
                      return (
                        <tr key={workspace.workspaceId} className="align-top transition-colors hover:bg-slate-50/60">
                          ...
                          <td className="min-w-[520px] px-5 py-4">
                            <div className="flex flex-wrap gap-2">
                              <form action={extendTrialAction} className="flex items-center gap-1.5"><input type="hidden" name="workspaceId" value={workspace.workspaceId} /><input name="days" type="number" min="1" max="365" defaultValue="7" className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" /><button className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-medium hover:bg-slate-50">Extend trial</button></form>
                              <form action={activateSubscriptionAction} className="flex items-center gap-1.5"><input type="hidden" name="workspaceId" value={workspace.workspaceId} /><select name="planCode" defaultValue={workspace.planCode || "starter"} className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs"><option value="starter">Starter</option><option value="business">Business</option><option value="pro">Pro</option></select><input name="days" type="number" min="1" max="1095" defaultValue="30" className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" /><button className="rounded-lg bg-[#059669] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#047857]">Activate</button></form>
                              <form action={grantGraceAction} className="flex items-center gap-1.5"><input type="hidden" name="workspaceId" value={workspace.workspaceId} /><input name="days" type="number" min="1" max="90" defaultValue="7" className="w-16 rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-xs" /><button className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-1.5 text-xs font-medium text-amber-800 hover:bg-amber-100">Grace</button></form>
