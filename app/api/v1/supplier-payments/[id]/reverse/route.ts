...
import { reverseSupplierPayment, SupplierPaymentReversalError } from "@/lib/server/supplier-payment-reversals";
...
export const POST = apiHandler(async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  try {
    return apiData(await reverseSupplierPayment({ ...context, userId: context.user.id }, id, body.reason));
  } catch (error) {
    if (error instanceof SupplierPaymentReversalError) throw new ApiError(422, "SUPPLIER_PAYMENT_REVERSAL_REJECTED", error.message);
    throw error;
  }
});
