...
export const POST = apiHandler(async (request: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const input = await parseApiBody(new Request(request.url, { method: "POST", headers: request.headers, body: JSON.stringify({ ...body, creditNoteId: id, idempotencyKey: key ?? body.idempotencyKey }) }), customerCreditAllocationSchema);
  try {
