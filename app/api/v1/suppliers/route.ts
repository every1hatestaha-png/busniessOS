...
export const POST = apiHandler(async (request: Request) => { const context = await requireApiContext("suppliers.manage"); const input = await parseApiBody(request, supplierSchema); return apiData(await createSupplier({ ...context, userId: context.user.id }, input), 201); });
