...
export const POST = apiHandler(async (request: Request) => {
  ...
  const input = await parseApiBody(new Request(request.url, { method: "POST", headers: request.headers, body: JSON.stringify({ ...body, idempotencyKey: key ?? body.idempotencyKey }) }), customerReturnSchema);
  try {
