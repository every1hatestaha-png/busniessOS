...
export function ReportFrame({ workspace, title, from, to, subtitle, filters, children, orientation = "portrait", printable = true, actions }: { workspace: WorkspaceDetails; title: string; from?: string | Date; to?: string | Date; subtitle?: string; filters?: ReactNode; children: ReactNode; orientation?: "portrait" | "landscape"; printable?: boolean; actions?: ReactNode }) {
  return (
    <div className="mx-auto min-w-0 max-w-[1400px] space-y-3 print:max-w-none print:space-y-0">
      ...
      {filters}
      <article data-print-surface data-print-orientation={orientation} className="min-w-0 overflow-hidden rounded-xl border border-neutral-200 bg-white p-4 shadow-sm sm:p-6 print:rounded-none print:border-0 print:p-0 print:shadow-none">
        <ReportCompanyHeader workspace={workspace} title={title} from={from} to={to} subtitle={subtitle} />
        <div className="mt-5 min-w-0 overflow-x-auto">{children}</div>
      </article>
    </div>
  );
}
