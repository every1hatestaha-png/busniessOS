...
import { DEMO_INVOICES, DEMO_KHATA_ENTRIES, DEMO_PAYMENTS, DEMO_SALES, type Customer } from "@/lib/demo-data";
...
export function CustomerDetailsTabs({ customer }: { customer: Customer }) {
  const [activeTab, setActiveTab] = useState<Tab>("Overview");
  const orders = DEMO_SALES.filter((sale) => sale.customerId === customer.id);
  const payments = DEMO_PAYMENTS.filter((payment) => payment.customerId === customer.id);
  const invoices = DEMO_INVOICES.filter((invoice) => invoice.customerId === customer.id);
  const ledger = calculateLedgerRunningBalance(DEMO_KHATA_ENTRIES.filter((entry) => entry.customerId === customer.id));
