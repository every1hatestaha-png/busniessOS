...
export function CustomerDetailsTabs({ customer }: { customer: CustomerDetail }) {
  ...
  for (const invoice of invoices) humanReferences.set(invoice.id, invoice.invoiceNumber);
  for (const payment of payments) humanReferences.set(payment.id, payment.reference && payment.reference !== "-" ? payment.reference : "Payment");
  const ledgerReference = (reference: string) => humanReferences.get(reference) ?? (uuidPattern.test(reference) ? "-" : reference);
