  ...
  return <form onSubmit={submit} className="grid items-start gap-4 2xl:grid-cols-[minmax(0,1fr)_320px]">
    <div className="space-y-4">
      <Card className="gap-0 rounded-md border py-0 shadow-none ring-0"><CardHeader className="border-b px-4 py-3"><CardTitle className="text-sm font-semibold">Supplier & Document</CardTitle></CardHeader><CardContent className="grid gap-4 p-4 md:grid-cols-3"><Field label="Supplier"><select required name="supplierId" className={fieldClass}><option value="">Choose supplier</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></Field><Field label="Department" hint="Optional"><Input name="department" placeholder="Department" /></Field><Field label="Expected delivery" hint="Optional"><Input name="expectedDeliveryDate" type="date" /></Field></CardContent></Card>
       ...
       <Card className="gap-0 rounded-md border py-0 shadow-none ring-0"><CardHeader className="flex-row items-center justify-between border-b px-4 py-3"><div><CardTitle className="text-sm font-semibold">Purchase Lines</CardTitle><p className="mt-0.5 text-[11px] text-slate-500">Choose unit or weight-based pricing for this order.</p></div><div className="flex items-center gap-2"><select aria-label="Pricing mode" value={pricingMode} onChange={(event) => changePricingMode(event.target.value as "UNIT" | "WEIGHT")} className={`${fieldClass} w-40`}><option value="UNIT">Unit pricing</option><option value="WEIGHT">Weight-based pricing</option></select><Button type="button" variant="outline" size="xs" onClick={() => setLines((current) => [...current, { productId: "", quantity: "", unitCost: "", unitWeight: "", perKgRate: "" }])}><Plus />Add line</Button></div></CardHeader><CardContent className="overflow-x-auto p-0"><div className="min-w-[1000px]">
