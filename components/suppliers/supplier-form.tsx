  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="rounded-xl border border-neutral-200 bg-white p-4 sm:p-5">
        <div className="mb-4"><h2 className="font-semibold">Supplier account</h2><p className="text-xs text-neutral-500">Contact and business information. Editing here never changes the payable balance.</p></div>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          {!supplier && <div className={fieldClassName}><label className={labelClassName} htmlFor="openingBalance">Opening payable balance (PKR)</label><Input id="openingBalance" type="number" min="0" step="0.01" {...register("openingBalance")} aria-invalid={!!errors.openingBalance} /><p className="text-xs text-amber-700">A positive amount creates the opening supplier liability.</p>{error("openingBalance")}</div>}
      ...
      <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end"><Link href={supplier ? `/suppliers/${supplier.id}` : "/suppliers"} className={buttonVariants({ variant: "outline", size: "sm" })}>Cancel</Link><Button type="submit" size="sm" disabled={isPending}>{isPending ? "Saving..." : supplier ? "Save changes" : "Add supplier"}</Button></div>
    </form>
  );
}
