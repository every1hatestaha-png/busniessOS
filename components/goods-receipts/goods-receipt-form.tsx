...
export function GoodsReceiptForm({ purchaseOrderId, poNumber, supplierName, items }: { purchaseOrderId: string; poNumber: string; supplierName: string; items: POItem[] }) {
  ...
  return <form onSubmit={submit} className="space-y-4">
    ...
    <Card className="gap-0 rounded-md border py-0 shadow-none ring-0"><CardHeader className="border-b px-4 py-3"><CardTitle className="text-sm font-semibold">Receiving Lines</CardTitle><p className="text-[11px] text-slate-500">Accepted quantity updates inventory. Weight-priced lines settle supplier liability from accepted kg x actual rate/kg.</p></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><div className="min-w-[1380px]">
      <div className="grid grid-cols-[minmax(230px,1fr)_80px_90px_90px_100px_100px_115px_115px_115px_125px] gap-2 border-b bg-slate-50 px-4 py-2 text-[10px] font-semibold uppercase tracking-wide text-slate-500"><span>Product</span><span className="text-right">Ordered</span><span className="text-right">Prev. accepted</span><span className="text-right">Remaining</span><span className="text-right">Received qty</span><span className="text-right">Accepted qty</span><span className="text-right">Received kg</span><span className="text-right">Accepted kg</span><span className="text-right">Actual rate</span><span className="text-right">Accepted value</span></div>
