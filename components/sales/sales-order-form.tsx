...
type ProductOption = { id: string; name: string; sku: string; sellingPrice: number; stockQuantity: number; reorderLevel: number; status: "ACTIVE" | "INACTIVE" | "ARCHIVED"; unit: string; defaultWeightKg: number | null; verifiedTaxRate: number | null };
...
export function SalesOrderForm({ customers, products, cashBankAccounts = [], canRecordPayments = true, warehouseMode = "LEGACY", warehouses = [], warehouseStocks = [] }: { customers: CustomerOption[]; products: ProductOption[]; cashBankAccounts?: CashBankOption[]; canRecordPayments?: boolean; warehouseMode?: "LEGACY" | "MANAGED"; warehouses?: WarehouseOption[]; warehouseStocks?: WarehouseStockOption[] }) {
  ...
  const { control, register, handleSubmit, getValues, setValue, formState: { errors, isSubmitting } } = useForm<OrderFormInput, unknown, OrderFormValues>({
  ...
  const gstRate = useWatch({ control, name: "gstRate" }) ?? 18;
  ...
  const availableStock = (productId: string) => warehouseMode === "MANAGED"
    ? (warehouseStocks.find((stock) => stock.warehouseId === warehouseId && stock.productId === productId)?.quantity ?? 0)
  ...
  const stockWarnings = items.flatMap((item) => {
    const product = products.find((entry) => entry.id === item?.productId);
    if (!product) return [];
    const warning = getSaleStockWarning(availableStock(product.id), Number(item?.quantity || 0), product.reorderLevel);
    return warning ? [{ productId: product.id, productName: product.name, ...warning }] : [];
  });
  const hasInsufficientStock = stockWarnings.some((warning) => warning.kind === "insufficient");
  ...
  async function loadCustomerPricing(nextCustomerId: string) {
    ...
    try {
      ...
      if (getValues("customerId") !== nextCustomerId) return;
      setPriceRules(rules);
  ...
  return <form onSubmit={handleSubmit(submitOrder)} className="mx-auto w-full max-w-[1500px] space-y-5 pb-10">
    ...
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      ...
      <div className="flex gap-2"><Button type="button" variant="outline" size="sm" render={<Link href="/sales" />}>Cancel</Button><Button type="submit" size="sm" disabled={isSubmitting || isPending || hasInsufficientStock}>{isPending ? "Creating..." : "Create Sale"}</Button></div>
    </header>
    ...
    {hasInsufficientStock && <div role="alert" className="flex items-start gap-2 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-red-800"><AlertCircle className="mt-0.5 size-4 shrink-0" /><p className="text-sm font-medium">Insufficient stock: {stockWarnings.filter((warning) => warning.kind === "insufficient").map((warning) => warning.productName).join(", ")}. Reduce quantity or choose another warehouse before submitting.</p></div>}
        ...
        <Card className="gap-0 overflow-hidden rounded-lg border py-0 shadow-sm ring-0">
          ...
          <CardContent className="overflow-x-auto p-0">
            ...
            {fields.map((field, index) => {
              ...
              const stockWarning = selectedProduct ? getSaleStockWarning(availableStock(selectedProduct.id), Number(line?.quantity || 0), selectedProduct.reorderLevel) : null;
              ...
              return <div key={field.id} className="grid grid-cols-[minmax(210px,1fr)_80px_110px_105px_120px_36px] items-start gap-2 border-b px-5 py-3 last:border-0">
                ...
                <Field error={errors.items?.[index]?.discountPerUnit?.message}><Input type="number" min="0" step="0.01" className="text-right text-sm" {...register(`items.${index}.discountPerUnit`, { valueAsNumber: true })} /></Field>
