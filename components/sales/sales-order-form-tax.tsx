...
const orderSchema = saleSchema.superRefine((order, context) => {
  ...
  const beforeTax = Math.max(0, gross - lineDiscounts - order.orderDiscount);
  const total = calculateSalesTax(beforeTax, order.taxEnabled, order.taxRate, order.taxMode).total;
...
export function SalesOrderFormTax({ customers, products, cashBankAccounts = [], canRecordPayments = true }: { customers: CustomerOption[]; products: ProductOption[]; cashBankAccounts?: CashBankOption[]; canRecordPayments?: boolean }) {
  ...
  const { control, register, handleSubmit, setValue, formState: { errors, isSubmitting } } = useForm<OrderFormInput, unknown, OrderFormValues>({
    ...
    defaultValues: {
      customerId: "",
      items: [{ productId: "", quantity: 1, pricingMode: "UNIT", unitWeight: undefined, perKgRate: undefined, unitPrice: 0, discountPerUnit: 0 }],
      orderDiscount: 0,
      taxEnabled: false,
      taxRate: 18,
      taxMode: "EXCLUSIVE",
  ...
  const taxEnabled = useWatch({ control, name: "taxEnabled" }) ?? false;
  const taxRate = useWatch({ control, name: "taxRate" }) || 0;
  const taxMode = useWatch({ control, name: "taxMode" }) ?? "EXCLUSIVE";
  const selectedCustomer = customers.find((customer) => customer.id === customerId);
  const subtotal = items.reduce((sum, item) => sum + (item?.quantity || 0) * (item?.unitPrice || 0), 0);
  const lineDiscounts = items.reduce((sum, item) => sum + (item?.quantity || 0) * (item?.discountPerUnit || 0), 0);
  const beforeTax = Math.max(0, subtotal - lineDiscounts - orderDiscount);
  const tax = calculateSalesTax(beforeTax, taxEnabled, taxRate, taxMode);
  ...
  function setPricing(index: number, mode: "UNIT" | "WEIGHT") {
    ...
    if (mode === "WEIGHT") {
      ...
      setValue(`items.${index}.unitPrice`, product.sellingPrice, { shouldValidate: true, shouldDirty: true });
    }
  }
  ...
  return <form onSubmit={handleSubmit(submitOrder)} className="mx-auto w-full max-w-[1500px] space-y-5 pb-10">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
      <div><Link href="/sales" className="mb-2 inline-flex items-center gap-1 text-xs font-medium text-slate-500 hover:text-slate-900"><ArrowLeft className="size-3.5" />Sales</Link><h1 className="text-2xl font-semibold tracking-tight text-slate-950">New Sales Order</h1><p className="mt-1 text-sm text-slate-500">Create the sale, invoice, receivable, tax liability, and stock movement in one step.</p></div>
    ...
    {Object.keys(errors).length > 0 && !actionState.error && <div role="alert" className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-amber-900"><AlertCircle className="mt-0.5 size-4 shrink-0" /><p className="text-sm font-medium">Please fix the highlighted order fields before creating the sale.</p></div>}
    ...
    <div className="grid min-w-0 items-start gap-5 2xl:grid-cols-[minmax(0,1fr)_340px]">
      <div className="min-w-0 space-y-5">
        ...
        <Card className="gap-0 overflow-hidden rounded-lg border py-0 shadow-sm ring-0">
          <CardHeader className="flex-row items-center justify-between border-b bg-slate-50/60 px-5 py-4"><div><CardTitle className="text-sm font-semibold">Line Items</CardTitle><p className="mt-0.5 text-xs text-slate-500">Add products, quantities, pricing, and per-unit discounts.</p></div><Button type="button" variant="outline" size="sm" onClick={() => append({ productId: "", quantity: 1, pricingMode: "UNIT", unitWeight: undefined, perKgRate: undefined, unitPrice: 0, discountPerUnit: 0 })}><Plus />Add line</Button></CardHeader>
